...
def step(context, link):
    ...
    context.clickActions = ClickActions(context)
    context.clickActions.click_text(link)
...
def step(context):
    """Then click logo icon8"""
    context.clickActions = ClickActions(context)
    context.clickActions.click_logo()
...
def step(context, button):
    """Then push link in navigation menu 'button'"""
    context.clickActions = ClickActions(context)
    context.clickActions.click_header_nav_menu(button)
...
def step(context, name):
    ...
    context.clickActions = ClickActions(context)
    context.clickActions.click_button_with_text(name)
...
def step(context):
    ...
    Then choose style of request icon
    """
    xpath = '//*[@ng-repeat="platform in icCtrl.platforms"][%s]' \
            % Random_generate.random_between_values(1, 5)
    context.clickActions = ClickActions(context)
    context.clickActions.click_on_xpath(xpath)
...
def step(context, button):
    """Then click '{button}' button"""
    context.clickActions = ClickActions(context)
    context.clickActions.click_button(button)
...
def step(context, type_of_buy):
    ...
    if type_of_buy == 'Free':
        ...
        button = """//div[@ng-bind-html="'PAGE.BUY.""" \
                 """PLANS.FREE.GET' | translate"]"""
    elif type_of_buy == 'All 33,100 Icons':
        move_to = '//*[@callback="buyHoverBlock(licenses.companies.updates)"]'
        button = """//*[@ng-bind-html="'PAGE.BUY.PLANS.BUY' | """ \
                 """translate"][@ng-class="licenses.companies.updates.id"]"""
    elif type_of_buy == 'Pay per Icon':
        buy_type = Value_generate.values_in_range(2, 4)
        move_to = '//div[@register-modal-params="{registerTitle:' \
                  'registerTitle, loginTitle:loginTitle}"][%s]' % buy_type
        button = '//div[@register-modal-params="{registerTitle:' \
                 'registerTitle, loginTitle:loginTitle}"][%s]/*[4]' % buy_type
    context.clickActions = ClickActions(context)
    context.clickActions.move_and_click(move_to, button)
...
def step(context, elements):
    ...
    if elements == 'collections':
        first_xpath = '//*[@ng-repeat="coll in vm.colls.colls" ][%s]' \
                     '/*[@ ng-show="collectionsEdit"]'
        second_xpath = '//*[@class="c-btn' \
                       ' modal__action-confirm modal__action"]'
    context.clickActions = ClickActions(context)
    context.clickActions.click_all_and_click_button(first_xpath, second_xpath)
...
def step(context, ng_if_locator):
    """Then click '{ng_if_locator}' button element"""
    context.clickActions = ClickActions(context)
    context.clickActions.click_ng_if_xpath(ng_if_locator)
...
def step(context, element):
    """Then check locate '{element}' element"""
    context.locateActions = LocateActions(context)
    context.locateActions.locate_locator(element)
...
def step(context):
    """Then check recently created icons"""
    xpath = '//a[@class="c-recently-icon"][%s]' \
...
def step(context):
    """Then check ideas list"""
    xpath = '//div[@class="b-idea-table-row"][%s]' \
            % Value_generate.values_in_range(1, 5)
    context.locateActions = LocateActions(context)
    context.locateActions.locate_element(xpath)
...
def step(context):
    ...
    file_read_text = File_actions.read_file(path_to_file)
...
def step(context, elements):
    ...
    if elements == 'Unlock Font':
        ...
        find_second = '//*[1][contains(text(), ' \
                      '"Paid or Free, You Are Our Hero!")]'
    ...
    context.locateActions = LocateActions(context)
    context.locateActions.try_find_click_find(try_find, click, find_second)


@then("locate '{element}' element in color pop-up")
def step(context, element):
    """Then locate '{element}' element in color pop-up"""
    if element == 'grayscale' or element == 'color' \
            or element == 'color' or element == 'color_palette'\
            or element == 'custom':
        xpath = '''//*[@class="colors"]/*/*[@ng-bind-html="'WEB_APP.''' \
                '''RECOLOR.%s' | translate"]''' % element.upper()

    elif element == 'gray colors':
        xpath = '//*[@class="colors"]/descendant::*[@ng-repeat="color_' \
                ' in colorsGray"][%s]' % Value_generate.values_in_range(1, 5)
    elif element == 'not gray colors':
        xpath = '//*[@class="colors-block"][2]/*[%s]' \
                % Value_generate.values_in_range(1, 10)
    elif element == 'canvas':
...
def step(context, field):
    ...
    context.textActions.input_text_id(text, field)
...
def step(context, input_text, locator_field):
    """Then add '{input_text}' text to '{locator_field}' field"""
    context.textActions = TextActions(context)
    context.textActions.add_text_to_field(input_text, locator_field)
...
def step(context, time):
    ...
    context.helpers.sleep_time(time)
